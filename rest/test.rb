# Automate svm on age range <18 or >47
# Output is in rest_results.txt
...
def f_svm(k, cval, jval, model_name, predict_file, pr_file)
	system "./svm_learn -t #{k} -c #{cval} -j #{jval} cs-training_rest_train.svm #{model_name}"
	system "./svm_classify cs-training_rest_test.svm #{model_name} #{predict_file}"
	system "python PR.py cs-training_rest_ref.svm #{predict_file} #{pr_file}"
	# get recall
...
outfile = File.new("rest_results.txt", "w")
R_sort.each do |elem|
	elem.each do |thing|
